...
int main()
{
	...
	{"x+y",	"a+b+c",	"a*(b+c)",	"(a+b)/(c-d)",	"a*(b+c)",	"a+(b-c^2)",	"1+2+3+x",	"1+2+3+x",	"x^3+x^2+x+1",	"x^3+x^2+x+1",	"sin(x)+y"};
	vector<string> dataTmpl = 
	{"A+B",	"A-B",		"A*B",		"A/B",			"A+B",		"A^2",			"1+A",		"2+x",		"A^2+1",		"x^3+1",		"A+sin(B)"};	
	vector<bool> ans = {true,false,true,true,true,true,true,true,true,true,true};
	vector<bool> myAns;
	int cntQ = ans.size();
	int cntCorrectA = 0;
